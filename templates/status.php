...
<div class="text-center">
	...
	<?php if ($user->published && $user->status === true) : ?>
		...
		<p><?=sprintf(__('Ti abbiamo mandato un\'email per informarti, ma se non fosse arrivata per favore contattata %1$s (<a href="https://t.me/%2$s">@%2$s</a>) su Telegram per passare al colloquio.'), $this->e($user->recruiter), $this->e($user->recruitertg))?></p>
...
<div class="text-center space-above">
	<a id="remove" class="btn btn-danger mb-2"
			href="<?=$this->e(WEEEOpen\WEEEHire\Utils::appendQueryParametersToRelativeUrl($globalRequestUri, ['delete' => 'true']))?>"><?=__('Elimina candidatura')?></a>
	<a id="download" class="btn btn-primary mb-2"
			href="<?=$this->e(WEEEOpen\WEEEHire\Utils::appendQueryParametersToRelativeUrl($globalRequestUri, ['download' => 'true']))?>"><?=__('Scarica tutti i miei dati')?></a>
</div>
<div class="text-center">
